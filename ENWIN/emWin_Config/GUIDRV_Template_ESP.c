...
/*********************************************************************
 打点函数
*/
static inline void ESP_SetPixelIndex (GUI_DEVICE* pDevice, int x, int y, int PixelIndex) {
	ESP_POINT_COLOR = PixelIndex;
...
*       ESP_FillRect
*/
static inline void ESP_FillRect (GUI_DEVICE* pDevice, int x0, int y0, int x1, int y1) {
...
*       ESP_DrawHLine
*/
static inline void ESP_DrawHLine (GUI_DEVICE* pDevice, int x0, int y, int x1) {
...
*/
static inline void ESP_DrawVLine (GUI_DEVICE* pDevice, int x, int y0, int y1) {
...
*/
static inline void ESP_DrawBitLine16BPP (GUI_DEVICE* pDevice, int x, int y, U16 const* p, int xsize) {
	ESP_LCD_SetCursor(x, y);
	*(__IO uint16_t *)(ESP_LCD_CMD) = eLCDPar_wramcmd; //写入命令
	for (; xsize > 0; xsize-- , x++ , p++) {
		*(__IO uint16_t *)(ESP_LCD_DATA) = (LCD_PIXELINDEX)*p;
	}
}
