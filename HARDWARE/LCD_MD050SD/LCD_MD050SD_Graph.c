...
void ESP_LCD_DrawPoint_big (u16 x_cur, u16 y_cur) {
	ESP_LCD_Fill(x_cur - 1, y_cur - 1, x_cur + 1, y_cur + 1, ESP_POINT_COLOR);
}

//在指定区域内填充指定颜色
//区域大小:
//  (xend-xsta)*(yend-ysta)
inline void ESP_LCD_Fill (u16 xsta, u16 ysta, u16 xend, u16 yend, u16 color) {
	u16 i, j;
	ESP_Address_set(xsta, ysta, xend, yend); //设置光标位置
	for (i = ysta; i <= yend; i++) {
		for (j = xsta; j <= xend; j++)
		ESP_LCD_WR_DATA(color);
		//设置光标位置
	}
}
...
void ESP_LCD_Draw_Line (u16 x0, u16 y0, u16 x1, u16 y1, u16 color) {
	u16 tempcolor = ESP_POINT_COLOR;
	ESP_POINT_COLOR = color;
	ESP_LCD_DrawLine(x0, y0, x1, y1);
	ESP_POINT_COLOR = tempcolor;
}
