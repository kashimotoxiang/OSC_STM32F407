...
u16 ESP_BACK_COLOR, ESP_POINT_COLOR; //背景色，画笔色

void ESP_LCD_WriteReg (u16 LCD_Reg, u16 LCD_RegValue) {
	ESP_LCD_WR_REG(LCD_Reg)

		;
	ESP_LCD_WR_DATA(LCD_RegValue)
...
void ESP_LCD_DisplayOn (void) {
	ESP_LCD_WriteReg(0x01, 3); //26íòéêa
}
...
void ESP_Address_set (u16 x1, u16 y1, u16 x2, u16 y2) {
	...
	ESP_LCD_WR_REG(0x000f);
}

/*  ---------------------------------------------------------*/
void ESP_LCD_Init (void) {
	ESP_LCD_RD_SET ;
	delay_ms(100); //′′|±￡¤òaóD1msòé
	ESP_LCD_WR_SET ;
	delay_ms(100); //′′|±￡¤òaóD1msòé
	ESP_LCD_REST_CLR ;
	delay_ms(100); //′′|±￡¤òaóD1msòé
...
//x,y:坐标
//返回值:此点的颜色
inline u16 ESP_LCD_ReadPoint (u16 x, u16 y) {
	if (x >= eLCDPar_width || y >= eLCDPar_height)
		return 0; //超过了范围,直接返回
	ESP_Address_set(x, y, x, y);
	return ESP_LCD_RD_DATA(); //这几种IC直接返回颜色值
}
