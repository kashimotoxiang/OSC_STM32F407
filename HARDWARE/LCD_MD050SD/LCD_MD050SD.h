...
//TFTLCD部分外要调用的函数
extern u16 ESP_POINT_COLOR;//默认红色
extern u16 ESP_BACK_COLOR; //背景颜色.默认为白色
//定义LCD的尺寸
...
#define	ESP_LCD_RS_SET	    GPIOE->ODR|=PIN3    //数据/命令      		PC8
#define	ESP_LCD_WR_SET	    GPIOD->ODR|=PIN5    //写数据				PC9
#define	ESP_LCD_RD_SET	    GPIOD->ODR|=PIN4   //读数据			 	PC10
#define	ESP_LCD_CS_SET      GPIOD->ODR|=PIN7   //片选端口  	    	PC11

#define	ESP_LCD_RS_CLR      GPIOE->ODR&=~PIN3   //片选端口  	     	PC8
#define	ESP_LCD_WR_CLR	    GPIOD->ODR&=~PIN5   //读数据			 	PC9
#define	ESP_LCD_RD_CLR	    GPIOD->ODR&=~PIN4   //写数据			 	PC10
#define	ESP_LCD_CS_CLR   	GPIOD->ODR&=~PIN7   //数据/命令         	PC11

#define	ESP_LCD_REST_SET	  ESP_LCD_REST_GPIO_Port->ODR|=ESP_LCD_REST_Pin   //复位     		        PC12
#define	ESP_LCD_REST_CLR	  ESP_LCD_REST_GPIO_Port->ODR&=~ESP_LCD_REST_Pin    //复位      	        PC12
/*  ---------------------------------------------------------*/
//PD0~15,作为数据线
#define ESP_LCD_BANK_ADDR                 ((uint32_t)0x60000000)
#define ESP_WRITE_READ_ADDR    						 ((uint32_t)0x100000)

#define ESP_LCD_CMD ESP_LCD_BANK_ADDR
#define ESP_LCD_DATA (ESP_LCD_BANK_ADDR+ESP_WRITE_READ_ADDR)

//读寄存器函数
#define ESP_LCD_RD_DATA()  *(__IO uint16_t*) (ESP_LCD_DATA)

//写地址
#define ESP_LCD_WR_REG(data)   *(__IO uint16_t*) (ESP_LCD_BANK_ADDR) = data
//写寄存器函数
#define ESP_LCD_WR_DATA(data)  *(__IO uint16_t*) (ESP_LCD_BANK_ADDR + ESP_WRITE_READ_ADDR) = data
...
extern u16 ESP_BACK_COLOR, ESP_POINT_COLOR; //背景色，画笔色
...
void ESP_ShowHanzi (unsigned int x_cur, unsigned int y_cur, unsigned char index);
...
void ESP_LCD_SetCursor (u16 Xpos, u16 Ypos);
